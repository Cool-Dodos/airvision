// ── Safe outdoor duration by AQI ─────────────────────────────────────────
export function safeOutdoorTime(aqi) {
...
// ── Best hour to go outside (simplified model) ────────────────────────────
// AQI is typically lowest early morning (before traffic/industry) and worst midday/evening
// Returns a human-readable recommendation
export function bestHourAdvice(aqi, dominentpol) {
  if (!aqi) return 'Check local forecast';
  if (aqi <= 50)  return 'Any time — air is clean';
  if (aqi <= 100) return 'Anytime, prefer 5–9 AM';

  // For ozone (O3) — worst in afternoon, best early morning
  if (dominentpol === 'o3') return '5–8 AM (ozone peaks 12–6 PM)';

  // For PM (traffic/industrial) — worst rush hours and evening
  if (dominentpol === 'pm25' || dominentpol === 'pm10')
    return '5–7 AM or after 8 PM';
...
export const POLLUTANT_EFFECTS = {
  pm25: {
    label:  'PM2.5',
    icon:   '🫁',
    source: 'Vehicles, industry, wildfires',
    effect: 'Penetrates deep into lungs and bloodstream. Linked to heart disease, lung cancer, and stroke.',
    color:  '#ff7e00',
  },
  pm10: {
    label:  'PM10',
    icon:   '💨',
    source: 'Dust, construction, road traffic',
    effect: 'Irritates airways and throat. Triggers asthma and bronchitis.',
    color:  '#ffaa00',
  },
  no2: {
    label:  'NO₂',
    ...
    effect: 'Inflames the lining of airways. Reduces lung function and worsens asthma.',
    color:  '#ff4444',
  },
  o3: {
    label:  'O₃ (Ozone)',
    icon:   '☀️',
    source: 'Formed from NO₂ + sunlight, vehicle emissions',
    effect: 'Chest pain and shortness of breath on exertion. Worsens in afternoon heat.',
    color:  '#aa44ff',
  },
  co: {
    label:  'CO',
    icon:   '🔥',
    source: 'Incomplete combustion, wildfires, vehicles',
    effect: 'Reduces oxygen delivery to organs. Headache, dizziness at high levels.',
    color:  '#884400',
  },
  so2: {
    label:  'SO₂',
    icon:   '⚗️',
    source: 'Coal burning, volcanic activity, refineries',
    effect: 'Irritates respiratory tract. Causes acid rain and worsens heart disease.',
    color:  '#ffcc00',
  },
};

// ── Pollution source tag from dominentpol ─────────────────────────────────
export const SOURCE_TAGS = {
  pm25: { tag: 'Industrial / Traffic',  icon: '🏭' },
  pm10: { tag: 'Dust / Construction',   icon: '🏗️' },
