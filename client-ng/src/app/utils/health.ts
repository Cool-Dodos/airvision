...
export const POLLUTANT_EFFECTS: Record<string, { label: string; icon: string; source: string; effect: string; color: string }> = {
  ...
  o3:   { label: 'O₃ (Ozone)', icon: '☀️', source: 'Formed from NO₂ + sunlight, vehicle emissions', effect: 'Chest pain and shortness of breath on exertion. Worsens in afternoon heat.', color: '#aa44ff' },
  ...
  so2:  { label: 'SO₂',        icon: '⚗️', source: 'Coal burning, volcanic activity, refineries', effect: 'Irritates respiratory tract. Causes acid rain and worsens heart disease.', color: '#ffcc00' },
};
